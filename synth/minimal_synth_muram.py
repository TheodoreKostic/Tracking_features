...
T = fits.open("muram_in.fits")[0].data
pgas = fits.open("muram_in.fits")[1].data
z = fits.open("muram_in.fits")[2].data
z = z[::-1]
vz = fits.open("muram_in.fits")[3].data
vturb = np.zeros(len(vz))
'''
atmos_in = np.loadtxt("falc.dat", skiprows = 1, overwrite = True)
z = atmos_in[1] / 1E2
T = atmos_in[2]
vz = np.zeros(len(z))
...
print("Time lw.Atmosphere has taken:{}".format(time.process_time()))
