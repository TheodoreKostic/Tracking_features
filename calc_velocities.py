...
for j in range(1, len(Temp)):
    ...
    Vm_T.append(vm)
    if (j//10 * 10 == j):
        print("info: processed ",j,"/",len(Temp))

print("info: horizontal velocities from T calculated")
...
for j in range(1, len(Bz)):
    ...
    Vm_Bz.append(vm)
    if (j//10 * 10 == j):
        print("info: processed ",j,"/",len(Bz))


print("info: horizontal velocities from Bz calculated")

vxhdu = fits.PrimaryHDU(Vx_mean)
vyhdu = fits.ImageHDU(Vy_mean)
vxthdu = fits.ImageHDU(Vel_x_T)
vythdu = fits.ImageHDU(Vel_y_T)
vxbzhdu = fits.ImageHDU(Vel_x_Bz)
vybzhdu = fits.ImageHDU(Vel_y_Bz)
hduarray = fits.hduarray([vxhdu, vyhdu, vxthdu, vythdu, vxbzhdu, vybzhdu])
hduarray.writeto(sys.argv[1])
...
plt.show()
>>>>>>> 6c167ee57dafee84443d97722621db6c096f80e8
